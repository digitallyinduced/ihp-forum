...
instance BuildMail NewCommentNotificationMail where
    subject = "IHP Forum: New Comment on " <> title
        where
            title = ?mail.thread.title
    to NewCommentNotificationMail { .. } = Address { addressName = Just (user.name), addressEmail = user.email }
    ...
    html NewCommentNotificationMail { .. } = [hsx|
        <body style="font-family: Helvetica, Arial, sans-serif; font-size: 16px; color: #073642; max-width: 600px">
            ...
            <strong style="font-size: 22px; color: #073642; font-weight: 800">IHP Forum: New Comment on <q>{thread.title}</q></strong>
            <p>
                {comment.body}
                <br />
                      ...
                      <table cellspacing="0" cellpadding="0">
                          <tr>
                              <td style="border-radius: 4px;" bgcolor="#0d6378">
                                  <a href={urlTo $ ShowThreadAction (thread.id)} target="_blank" style="padding: 8px 12px; border: 1px solid #0948581a;border-radius: 2px;font-family: Helvetica, Arial, sans-serif;font-size: 14px; color: #ffffff;text-decoration: none;font-weight:bold;display: inline-block;">
